...
const getSaleforceAccounts = async (accessToken) => {
  let config = {
    method: "get",
    maxBodyLength: Infinity,
    url: `https://${salesforce.organsation}.my.salesforce.com/services/data/v52.0/query?q=SELECT id, Name, BillingAddress FROM Account`,
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  };
...
const createSalesforceAccount = async (accessToken, data) => {
  let config = {
    method: "post",
    maxBodyLength: Infinity,
    url: `https://${salesforce.organsation}.my.salesforce.com/services/data/v60.0/sobjects/Account/`,
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
    data
